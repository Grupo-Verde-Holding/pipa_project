...
if df_m.empty:
    ...
    # ── KPI de Manutenção ────────────────────────────────────────────────────
    total_reg = len(df_m)
    st.metric("Registros no período", total_reg)
    ...
    st.subheader("Serviços por Veículo")
    ranking = (
        df_m.groupby("veiculo")
        .agg(servicos=("tipo", "count"))
        .sort_values("servicos", ascending=False)
        .reset_index()
    )
    ranking.columns = ["Placa", "Qtd Serviços"]
    ...
    df_exibir = (
        df_m[["Veículo", "tipo", "data", "KM", "descricao"]]
        .head(10)
        .rename(columns={
